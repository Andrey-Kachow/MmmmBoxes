...
function filteredSearchDisplayAlter(
    inputId, containerId, listItemTagName, textGetter) {
  const filter = document.getElementById(inputId).value
  const items =
      document.getElementById(containerId).getElementsByTagName(listItemTagName)
...
function filterResidents() {
  filteredSearchDisplayAlter(
      'resident_name_inp', 'dropdown_residents', 'button',
      (elem) => elem.textContent)
}

function filterListOfresidents() {
  filteredSearchDisplayAlter(
      'resident_list_name_inp', 'list_of_residents', 'li',
      (elem) => elem.textContent)
}
